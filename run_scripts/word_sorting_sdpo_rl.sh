...
# Word sorting SDPO + CISPO student RL with gradient norm logging.
#
# Combines the proven CISPO RL setup from word_sorting_rl.sh with SDPO
# distillation. Uses sdpo_loss_coef=0.02 and rl_grad_balance=log to
# separately log SDPO and RL gradient norms for tuning.
#
...
#
# Usage: ./run_scripts/word_sorting_sdpo_rl.sh [experiment_name_suffix]
...
if [ -n "$EXP_NAME_OVERRIDE" ]; then
    ...
    EXP_NAME="LOCAL-WORDSORT-SDPO-RL-train${TRAIN_BATCH_SIZE}-alpha${ALPHA}-rollout${ROLLOUT_BATCH_SIZE}-lr${LR}-${MODEL_NAME}-${SUFFIX}"
fi
...
echo "----------------------------------------------------------------"
echo "Starting Word Sorting SDPO+CISPO RL Training"
