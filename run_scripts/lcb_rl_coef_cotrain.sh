...
# LCB SDPO + RL + teacher co-training with forward KL.
#
# Extends lcb_rl_coef.sh by adding teacher RL co-training:
...
#
# Usage: ./run_scripts/lcb_rl_coef_cotrain.sh [experiment_name_suffix]
...
if [ -n "$EXP_NAME_OVERRIDE" ]; then
    ...
    EXP_NAME="LOCAL-SDPO-RL-COTRAIN-train${TRAIN_BATCH_SIZE}-alpha${ALPHA}-rollout${ROLLOUT_BATCH_SIZE}-lr${LR}-lambda${LAMBDA}-clip_adv_high${CLIP_ADV_HIGH}-dross${DONTS_REPROMPT_ON_SELF_SUCCESS}-${MODEL_NAME}-${SUFFIX}"
fi
...
echo "----------------------------------------------------------------"
echo "Starting LCB SDPO + RL + Teacher Co-training"
