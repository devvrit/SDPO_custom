...
# SDPO "minimal edit" variant of CodeIO Mini.
#
...
# on the feedback.  This uses the `include_student_response_in_reprompt` flag
# so that {student_response} is populated in the reprompt template.
#
# Usage: ./run_scripts/codeio_mini_edit.sh [experiment_name_suffix]
...
if [ -n "$EXP_NAME_OVERRIDE" ]; then
    ...
    EXP_NAME="LOCAL-CODEIO-MINI-EDIT-train${TRAIN_BATCH_SIZE}-alpha${ALPHA}-rollout${ROLLOUT_BATCH_SIZE}-lr${LR}-${MODEL_NAME}-${SUFFIX}"
fi
...
echo "----------------------------------------------------------------"
echo "Starting CodeIO Mini EDIT SDPO Training"
...
bash "$PROJECT_ROOT/training/verl_training.sh" "$EXP_NAME" "$CONFIG_NAME" "$DATA_PATH" $FINAL_ARGS \
    $'actor_rollout_ref.actor.self_distillation.reprompt_template="{prompt}{solution}{feedback}\n\nThe following is the student\'s response to the problem:\n\n{student_response}\n\nRe-write the student\'s response above with minimal edits to correct it, accounting for the feedback. Keep the response structure and reasoning intact \u2014 only change what is necessary to arrive at the correct answer. Provide your final answer as a JSON value (object, list, number, or boolean).\n"'
