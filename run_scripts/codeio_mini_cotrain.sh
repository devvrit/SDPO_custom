...
# Minimal fast SDPO validation setup using the CodeIO task from reasoning_gym.
#
# CodeIO: given a Python function and input values, predict the output as JSON.
# Reward: reasoning_gym's JSON tree edit distance scorer + field-level feedback.
#
# Key differences from lcb.sh:
#   - max_response_length=4096  (vs 16384 for LCB — much faster rollouts)
#   - max_prompt_length=2048    (codeio prompts are ~300–600 tokens)
#   - trainer.total_epochs=3    (120 train problems / 8 batch ≈ 45 steps)
#   - No code sandbox           (prediction only — instant reward computation)
#
# Usage: ./run_scripts/codeio_mini.sh [experiment_name_suffix]
...
ALPHA=1.0                  # reverse KL (good for learning from successes)
...
if [ -n "$EXP_NAME_OVERRIDE" ]; then
    ...
    EXP_NAME="LOCAL-CODEIO-MINI-train${TRAIN_BATCH_SIZE}-alpha${ALPHA}-rollout${ROLLOUT_BATCH_SIZE}-lr${LR}-${MODEL_NAME}-${SUFFIX}"
fi
...
echo "----------------------------------------------------------------"
echo "Starting CodeIO Mini SDPO Training"
...
bash "$PROJECT_ROOT/training/verl_training.sh" "$EXP_NAME" "$CONFIG_NAME" "$DATA_PATH" $FINAL_ARGS \
    $'actor_rollout_ref.actor.self_distillation.reprompt_template="{prompt}{solution}{feedback}\n\nNow carefully re-examine the code and predict the correct output. Provide your final answer as a JSON value (object, list, number, or boolean).\n"'
