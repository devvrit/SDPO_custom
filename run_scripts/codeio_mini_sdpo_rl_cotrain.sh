...
# CodeIO SDPO + CISPO student RL + teacher co-training with forward KL.
#
# Extends codeio_mini_sdpo_rl.sh by adding teacher RL co-training:
...
#
# Usage: ./run_scripts/codeio_mini_sdpo_rl_cotrain.sh [experiment_name_suffix]
...
if [ -n "$EXP_NAME_OVERRIDE" ]; then
    ...
    EXP_NAME="LOCAL-CODEIO-SDPO-RL-COTRAIN-train${TRAIN_BATCH_SIZE}-alpha${ALPHA}-rollout${ROLLOUT_BATCH_SIZE}-lr${LR}-${MODEL_NAME}-${SUFFIX}"
fi
...
echo "----------------------------------------------------------------"
echo "Starting CodeIO SDPO+CISPO RL + Teacher Co-training"
