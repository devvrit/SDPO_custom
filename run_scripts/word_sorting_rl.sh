...
# Vanilla RLVR baseline (GRPO + CISPO) for word_sorting — no self-distillation.
#
# Usage: ./run_scripts/word_sorting_rl.sh [experiment_name_suffix]
...
if [ -n "$EXP_NAME_OVERRIDE" ]; then
    ...
    EXP_NAME="LOCAL-WORDSORT-RL-train${TRAIN_BATCH_SIZE}-rollout${ROLLOUT_BATCH_SIZE}-lr${LR}-${MODEL_NAME}-${SUFFIX}"
fi
...
echo "----------------------------------------------------------------"
echo "Starting Word Sorting RL (GRPO + CISPO) Training"
