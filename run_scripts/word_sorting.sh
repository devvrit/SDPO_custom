...
# Word sorting SDPO training with Qwen3-1.7B.
#
# word_sorting: given a list of words, sort them in ascending/descending ASCII order.
# Reward: reasoning_gym position-based partial credit + detailed positional feedback.
#
# Usage: ./run_scripts/word_sorting.sh [experiment_name_suffix]
...
if [ -n "$EXP_NAME_OVERRIDE" ]; then
    ...
    EXP_NAME="LOCAL-WORDSORT-train${TRAIN_BATCH_SIZE}-alpha${ALPHA}-rollout${ROLLOUT_BATCH_SIZE}-lr${LR}-${MODEL_NAME}-${SUFFIX}"
fi
...
echo "----------------------------------------------------------------"
echo "Starting Word Sorting SDPO Training"
