...
# Vanilla RLVR baseline (GRPO + CISPO) for word_sorting — THINKING MODE enabled.
#
# Based on word_sorting_rl.sh with these changes for thinking:
...
#   - max_model_len: 12288 (no reprompting, so smaller than SDPO variant)
#
# Usage: ./run_scripts/word_sorting_rl_think.sh [experiment_name_suffix]
...
MAX_MODEL_LEN=12288       # prompt + thinking response (no reprompting in RL)
...
if [ -n "$EXP_NAME_OVERRIDE" ]; then
    ...
    EXP_NAME="LOCAL-WORDSORT-RL-THINK-train${TRAIN_BATCH_SIZE}-rollout${ROLLOUT_BATCH_SIZE}-lr${LR}-${MODEL_NAME}-${SUFFIX}"
fi
...
echo "----------------------------------------------------------------"
echo "Starting Word Sorting RL (GRPO + CISPO) Training (THINKING MODE)"
