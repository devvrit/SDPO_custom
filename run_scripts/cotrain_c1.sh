...
# Usage: ./run_scripts/cotrain.sh [experiment_name_suffix]
...
#
# Like general.sh but enables teacher-context RL loss (teacher_rl_loss_coef)
# alongside the SDPO distillation loss. The teacher is the student model
# conditioned on reprompted inputs; IS correction accounts for context mismatch.
...
actor_rollout_ref.actor.self_distillation.teacher_rl_is_clip=$TEACHER_RL_IS_CLIP \
...
echo "----------------------------------------------------------------"
echo "Starting SDPO + Teacher RL Co-Training"
