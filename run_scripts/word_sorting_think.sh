...
# Word sorting SDPO training with Qwen3-1.7B — THINKING MODE enabled.
#
# Based on word_sorting.sh with these changes for thinking:
...
#   - remove_thinking_from_demonstration: True (default, strips <think> from demos)
#
# Usage: ./run_scripts/word_sorting_think.sh [experiment_name_suffix]
...
if [ -n "$EXP_NAME_OVERRIDE" ]; then
    ...
    EXP_NAME="LOCAL-WORDSORT-THINK-train${TRAIN_BATCH_SIZE}-alpha${ALPHA}-rollout${ROLLOUT_BATCH_SIZE}-lr${LR}-${MODEL_NAME}-${SUFFIX}"
fi
...
echo "----------------------------------------------------------------"
echo "Starting Word Sorting SDPO Training (THINKING MODE)"
